        ...
        if None != re.search('/block/*', self.path):
            if None != re.search('/block/getBlockData', self.path):
               ...
               # 블록 요청 from에 음수값, 0값 예외 처리
                ...
                for i in block :
                    maxIndex = int(block.index(i))

                if startPoint < 0 : # 시작점이 음수일 경우 0부터 시작
                    startPoint = 0
                    print("unexpacted requests, startPoint has set 0.")

                if endPoint > maxIndex : # 초과 요청 시
                    endPoint = maxIndex
                    print("endPoint has set last index of block. we send till " + str(maxIndex) + " blocks.")
                ...
                try:
                    ...
                    self.send_header('Content-type', 'application/json')
                    self.end_headers()
                    data.append("Errors occured : index out of range, check it again")
                finally:
